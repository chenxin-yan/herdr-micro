...
**What to build:** Both supported install paths produce the same working Host, launchd service, and Deck setup from the same source. Manual onboarding is `bunx herdr-micro setup`: it compiles and installs a stable Host binary, registers the `dev.herdr.herdr-micro` agent in the `gui` launchd domain, and provisions the Deck without creating a config. Only `herdr-micro config init` creates one. `herdr-micro up` and `down` enable/start and stop/disable that agent. `herdr-micro uninstall` removes a CLI-managed service registration plus the installed binary, build workspace, shim, and download cache; it preserves config, logs, and the Deck and refuses to alter a plist without the herdr-micro ownership marker.
...
- [ ] `bunx herdr-micro setup` installs a stable Host without creating or changing config, registers and starts the marked `dev.herdr.herdr-micro` launchd agent, provisions the Deck, and is safe to rerun
